...
2. Fill in the fields:
   ...
   **Required:**
   ...
   - `role` — must be exactly one of: `"leadership"`, `"postdoc"`, `"visiting"`, `"affiliate"`, or `"alumni"`. This controls which filter group they show up in (Leadership / Postdoctoral Fellows / Visiting Fellows / Affiliates / Alumni). Any other value will cause them to not appear when a filter is selected. Use `"alumni"` for anyone who has left CES (e.g. a former postdoc who has moved on) rather than deleting their entry — update their `title` to say "Former ..." so it's clear on their card and profile. Use `"affiliate"` for JHU faculty (from any department/school) who are affiliated with CES but aren't core CES leadership, postdocs, or visiting fellows.
...
- Remember `"role"` must be exactly `"leadership"`, `"postdoc"`, `"visiting"`, `"affiliate"`, or `"alumni"` — any other spelling will not show up under any filter.
